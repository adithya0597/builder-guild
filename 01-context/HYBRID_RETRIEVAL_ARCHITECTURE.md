...
5. **Embed — two granularities:** (a) embed **long_context** (the description) → node-level vector for coarse "which node" recall, stored as `:Entity.embedding` (the `node_embedding` HNSW index); (b) embed each **chunk** of a multi-chunk node → chunk-level vector for fine "which passage" recall, stored one-per-chunk on a **`:Chunk` node** linked `(:Entity)-[:HAS_CHUNK]->(:Chunk)` (the `chunk_embedding` HNSW index). EmbeddingGemma-300M local ($0) default; gemini-embedding-001 when quality > cost. **Why `:Chunk` nodes, not a vector-list on `:Entity` (cf7, shipped):** Neo4j HNSW indexes exactly ONE vector property per node, so multi-chunk recall needs one indexed vector PER chunk — i.e. a node per chunk. (This supersedes an earlier "vectors stored on the node, single store" phrasing, which collided with that one-vector-per-node index reality.) A **single-chunk** node gets NO `:Chunk` node — its passage IS its `long_context`, returned directly (§3; bzr).
...
| **2b. Chunk-vector** (fine) | `:Chunk.embedding` (one `:Chunk` node per chunk of a multi-chunk node; `chunk_embedding` index) | "which passage" inside candidate nodes; single-chunk node → skip, return directly | HNSW, cheap | Neo4j `CREATE VECTOR INDEX … ON (c:Chunk).embedding` (cf7) |
