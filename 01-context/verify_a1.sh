...
set -uo pipefail
cd "$(cd "$(dirname "$0")" && pwd)"
echo "== recreate bg-neo4j with fixed config =="
docker compose down --remove-orphans 2>&1 | tail -3
docker compose up -d 2>&1 | tail -10
echo "== wait for neo4j to accept cypher (<=180s) =="
ok=0
for i in $(seq 1 36); do
  if docker exec bg-neo4j cypher-shell -u neo4j -p companybrain "RETURN 1;" >/dev/null 2>&1; then
...
echo "== edition =="
docker exec -i bg-neo4j cypher-shell -u neo4j -p companybrain --format plain <<'CYPHER' 2>&1
...
docker exec -i bg-neo4j cypher-shell -u neo4j -p companybrain <<'CYPHER' 2>&1 | tail -6
...
docker exec -i bg-neo4j cypher-shell -u neo4j -p companybrain <<'CYPHER' 2>&1 | tail -2
...
docker ps --filter name=bg-neo4j --format '{{.Names}} | {{.Status}} | {{.Image}}'
docker volume ls --filter name=bg_neo4j --format '{{.Name}}'
echo "A1_VERIFY_DONE"
