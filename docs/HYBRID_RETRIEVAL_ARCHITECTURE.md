...
Scope: **the agent-infrastructure knowledge base only** — the "company brain" the agent fleet builds, manages, and retrieves from. NOT a personal note-taking system. **Zero personal-KB primitives** by lock. Only the design *pattern* + shared evidence base carry over.
...
3. **Node-property schema** — description tier + content tier are DISTINCT:
   ...
   - **long_context** — *longer description*: a fuller summary/abstract of the node (more than short; still a description, **NOT** the raw chunks). → node-level vector recall (embed this) + mid-tier injection. **PageIndex does NOT use `long_context`** — it builds its own ToC tree over `chunks`/source doc via `pageindex_ref` (PART 2). [residual fix, post-Hermes-review]
...
- **Reasoning/multi-hop** → **CoT decompose** → per-sub-question: graph traversal + vector → **CRAG evaluator** (re-search low-confidence) → aggregate → rerank. (Cosine alone 18.3 nDCG on reasoning; CoT lifts BM25 14.8→26.5 — BRIGHT; CRAG +7–36.6%.)
...
Evidence schema (at ingest, every candidate): `evidence_id · source_id · source_path · source_type · node_id · chunk_span · retrieved_at · valid_at · invalid_at · created_at · expired_at · freshness_state · namespace · retrieval_method · raw_score · confidence`.
